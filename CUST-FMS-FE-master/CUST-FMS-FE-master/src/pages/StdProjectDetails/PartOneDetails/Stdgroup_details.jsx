...
const Group_details = ({ data, accordionId }) => {
  ...
  useEffect(() => {
    const fetchFYPData = () => {
      try {
        // Retrieve userFypRegistration from localStorage
        const userFypRegistrationString = localStorage.getItem('userFypRegistration');
  
        // Parse the string to convert it back to an object
        const userFypRegistration = JSON.parse(userFypRegistrationString);
  
        // Check if userFypRegistration is not null or undefined
        if (userFypRegistration) {
          // Set the fetched data to your state variable
          setFypData(userFypRegistration);
        } else {
          console.error('userFypRegistration not found in localStorage.');
        }
      } catch (error) {
        console.error('Error fetching userFypRegistration from localStorage:', error);
      }
    };
