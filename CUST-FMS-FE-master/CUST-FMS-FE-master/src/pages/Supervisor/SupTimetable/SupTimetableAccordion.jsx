...
//     // If in edit mode, allow editing
//     if (editingMode) {
//       // If the content is '', clear it
...
//     // If in edit mode, allow updating content
//     if (editingMode) {
//       // If the content is empty, restore ''
//       if (!cell.innerText.trim()) {
//         cell.innerText = '';
//       }
...
//     // Allow editing initially, then disable after confirmation
//     setEditingMode(true);

//     // If timetable has been added before, set timetableUpdated to true after editing
//     if (timetableAdded) {
...
//       // Set timetableAdded to true after adding timetable
//       setTimetableAdded(true);
//       // Reset timetableUpdated after showing the confirmation for editing
...
//         >
//           <span>{timetableAdded ? 'Timetable' : 'Add Timetable'}</span>
//           <svg
//             data-accordion-icon
...
//               {timetableUpdated ? 'Timetable updated successfully' : 'Timetable uploaded successfully'}
//             </div>
//           )}
//           {/* Timetable Table */}
//           <div className="table-container overflow-x-auto relative">
//             {showEditButton && (
//               <div className="ml-[1510px] cursor-pointer" onClick={handleEditButtonClick}>
...
//                     <td
//                       key={index}
//                       className={`px-6 py-4 text-center border-r ${editingMode ? 'editable' : ''}`}
...
const TimetableAccordion = ({ accordionId }) => {
  ...
  const [TimetableUpdatedMsg, /* setTimetableUpdatedMsg */] = useState(false); // unused setTimetableUpdatedMsg
  ...
  const handleUpdateButtonClick = async () => {
    try {
      ...
      const mergedTimetableData = { ...TimetableData };

      // Update the merged data with the newly entered data
      days.forEach((day) => {
        timeSlots.forEach((slot, slotIndex) => {
          const key = `${day}_${slotIndex + 8}:00`;
          const classValue = cellContents[key] || '';
          // Update the class value only if it's not empty
          if (classValue !== '') {
            // Update the slot if it exists in the merged data
            if (mergedTimetableData[day] && mergedTimetableData[day][slotIndex]) {
              mergedTimetableData[day][slotIndex].class = classValue;
            } else {
              // Create a new slot if it doesn't exist in the merged data
