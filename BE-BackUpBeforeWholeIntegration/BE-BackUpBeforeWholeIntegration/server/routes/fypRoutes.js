...
router.delete(
  "/DeleteWholeProject/:groupId",
  authMiddleware,
  fypRegController.deleteFypRegistrationProject
);
router.delete(
  "/Group/:groupId/students/:studentId",
  authMiddleware,
  fypRegController.removeStudentFromFypProject
);

router.get(
  "/GetStudentCount/:deptId",
  authMiddleware,
  fypRegController.StudentCount
);
router.get(
  "/GetStudentOfTerm/:termId",
  authMiddleware,
  fypRegController.fetchStudentsByTerm
);
router.get("/GetProjectCount", authMiddleware, fypRegController.ProjectCount);
router.delete(
...
// Supervisor Topic/Tech Change Request Routes
router.get(
  "/topicChangeReqsForSupervisor/:supervisorId",
  authMiddleware,
  TopicReqController.getTopicReqsForSupervisor
);

router.patch(
  "/approveTopicRequest/:id",
  authMiddleware,
  TopicReqController.approveTopicRequest
);

router.patch(
  "/rejectTopicRequest/:id",
  authMiddleware,
  TopicReqController.rejectTopicRequest
);
...
router.get("/getfypregistrationsforStudent/:userid", authMiddleware, async (req, res) => {
  try {
    await fypRegController.getfypregistrationsforStudent(req, res);
  } catch (error) {
    console.error("Error in getfypregistrationsforStudent route:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
});
